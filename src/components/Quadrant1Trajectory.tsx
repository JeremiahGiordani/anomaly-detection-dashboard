...
export default function Quadrant1Trajectory() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((d) => {
      const passTime = !timeRange || (d.t >= timeRange.min && d.t <= timeRange.max);
      const passAlt = !altRange || (d.alt >= altRange.min && d.alt <= altRange.max);
      return passTime && passAlt;
    });
  }, [data, timeRange, altRange]);

  if (data.length === 0) return <p className="text-gray-600">Loading trajectory…</p>;
